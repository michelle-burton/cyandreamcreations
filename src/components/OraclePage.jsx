...
const cards = [...source.matchAll(/^### ([0IVX]+) · (.*?) — (.*?)\n([\s\S]*?)(?=\n### |\n---)/gm)].map((match) => {
  ...
  const padded = match[1] === '0' ? '00' : String(['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI','XVII','XVIII','XIX','XX','XXI'].indexOf(match[1]) + 1).padStart(2, '0')
...
export default function OraclePage() {
  ...
  return <main className={`oracle-sanctuary ${stage === 'ritual' ? 'oracle-at-threshold' : ''}`}>
    ...
    {stage === 'reading' && selected.length === 3 && <section className="oracle-reading">
      ...
      {!isReading && <>
      <div className="oracle-three-cards">{selected.map((card, i) => <article key={card.numeral} style={{ '--house-light': card.house[0].color }}><p className="oracle-eyebrow">{positions[i]}</p><div className="oracle-revealed"><span>{card.numeral}</span>{card.art && <img className="oracle-card-art" src={card.art} alt={`${card.name} oracle card`} />}<h2>{card.name}</h2><p>{card.house.length === 4 ? 'All four Houses' : `House of ${card.house[0].name}`}</p></div><div className="oracle-card-whisper" style={{ '--reveal-delay': `${i * 150 + 350}ms` }}><span className="oracle-whisper-star" aria-hidden="true">✦</span><p className="oracle-card-message">{card.message}</p><p className="oracle-card-reflection">{card.reflection}</p><div className="oracle-card-invitation"><h3>A small invitation</h3><p>{card.invitation}</p></div></div></article>)}</div>
      </>}
